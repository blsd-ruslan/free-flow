...
export class Grid extends Element {
  ...
  private bindEvents(): void {
    this.on('pointerdown', (event) => {
      ...
      path.push(cell);
      this.#paths.set(<number>path.colour(), path);
    });
    ...
    this.on('pointerup', (event) => {
      ...
      if (this.#currentPath.length() < 2) {
        ...
        this.#paths.delete(<number>this.#currentPath.colour());
      }
  ...
  private pathFromCell(
    ...
    if (
      path === null &&
      cell instanceof Point &&
      this.#paths.has(<number>cell.colour())
    ) {
      path = this.#paths.get(<number>cell.colour());
